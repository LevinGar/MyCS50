...
def fullMapView(request):
    posts = Post.objects.all().values('lat','lng')
    return JsonResponse({'res': list(posts)})

def getAddress(latlng):
    params = {
    'latlng': latlng,
    'key': API_KEY,
    'result_type|':'RANGE_INTERPOLATED'
    }
    res = requests.get(base_url,params=params)
    data=res.json()
    forma = data['results'][0]['formatted_address']
    return forma
...
def newpost(request, username):
    if request.method == 'POST':
        ...
        if not textarea or not coor:
            return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
        dir = getAddress(coor)
        post = Post.objects.create(content=textarea, user=user,address=dir,lat=lat,lng=lng,requester=req)
        post.save()
        return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
...
def following(request, username):
    if request.method == 'GET':
        ...
        if not follows:
            return render(request, 'network/following.html', {'message': "No sigues a nadie"})
