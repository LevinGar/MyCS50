$(function() {
    ...
    socket.on('connect', () => {
        $('#messageInput').on("keyup", function(key) {
            ...
            if (key.keyCode == 13 && $(this).val() != "" && !privateWindow && !inRoom) { //broadcast to all
            ...
            if (key.keyCode == 13 && $(this).val() != "" && !privateWindow && inRoom) { //send to room
                const mymessage = $(this).val();
                const username = localStorage.getItem('username');
                const time = new Date().toLocaleString();
                $('#messageInput').val("")
            ...
            } else if (key.keyCode == 13 && $(this).val() != "" && privateWindow && !inRoom) { //send private
                const mymessage = $(this).val();
                const username = localStorage.getItem('username');
        ...
        $('#channelList').on('click', 'li', function() {
            ...
            socket.emit('join', {
                'channel': activeChannel,
                'mymessage': 'has entered the room',
                'username': username,
                'time': time
            });
        });
        ...
        if (!localStorage.getItem('username')) {
            ...
            $('.modal-title').text("Set your username");
            $('#modalInput').val("");
        }
    });
    ...
    socket.on('add channel', data => {
        if (data["error"] != "") {
            ...
            $('#messageInput').focus();
            socket.emit('update users channels', {
    ...
    $('kbd').on('click', function() {
        ...
        $('.modal-title').text("Enter channel name");
        $('#modalInput').val("");
        $("#modalButton").attr('disabled', true);
    });
